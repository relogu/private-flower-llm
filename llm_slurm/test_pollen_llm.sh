...
#! Test NodeManager
HYDRA_FULL_ERROR=1 poetry run python -m pollen_worker.server_with+pollen $LLM_OPTIONS pollen.saving_path=$SAVE_PATH 2>&1 | tee $SAVE_PATH/server.log &

HYDRA_FULL_ERROR=1 poetry run python -m pollen_worker.node_manager.node_manager $LLM_OPTIONS pollen.saving_path=$SAVE_PATH 2>&1 | tee $SAVE_PATH/node_manager.log 
