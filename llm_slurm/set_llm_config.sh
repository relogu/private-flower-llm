...
if [[ $GPU_TYPE == *'A40'* ]]; then
    ...
    LLM_CONFIG_MPT_15M="llm_config=mpt-15m llm_config.device_train_microbatch_size=64 llm_config.device_eval_batch_size=50"
    ...
    LLM_CONFIG_MPT_15M="llm_config=mpt-15m llm_config.device_train_microbatch_size=128 llm_config.device_eval_batch_size=50"
...
if [[ "$1" == "small" ]]; then
    ...
    export LLM_CONFIG=$LLM_CONFIG_MPT_15M
elif [[ "$1" == "125M" ]]; then
    ...
    echo "Valid input arguments are: small, 15M, 125M, 350M, 760M, 1B, 3B, 7B, 70B"
    exit 1
fi
