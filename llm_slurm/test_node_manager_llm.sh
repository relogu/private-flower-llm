...
LLM_OPTIONS="llm_config.train_loader.dataset.split=train_small llm_config.eval_loader.dataset.split=val_small llm_config.data_local=$DATA_ROOT_SMALL llm_config.device_train_microbatch_size=20 llm_config.save_interval=10ba llm_config.save_num_checkpoints_to_keep=1 llm_config.save_folder=$SAVE_PATH llm_config.train_loader.num_workers=12 llm_config.eval_loader.num_workers=12 llm_config.max_duration=10ba llm_config.autoresume=True" # llm_config.load_path=$SAVE_PATH/ckpt-0.pt"
...
# HYDRA_FULL_ERROR=1 poetry run composer $COMPOSER_OPTIONS pollen_worker/node_manager/node_manager.py $LLM_OPTIONS is_test=true
