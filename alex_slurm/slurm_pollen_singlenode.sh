...
poetry shell
cd /nfs-share/aai30/projects/pollen_worker/src

# Clean the shared memory objects
poetry run python clean_memory.py
...
poetry run python server_with+pollen.py $CUSTOM_HYDRA_ARGS & # >> server_$timestamp.out 2>&1 &
...
poetry run python pure_sh_node_manager.py $CUSTOM_HYDRA_ARGS # >> node_manager_$timestamp.out 2>&1
