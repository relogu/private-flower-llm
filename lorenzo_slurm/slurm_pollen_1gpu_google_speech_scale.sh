...
#SBATCH -w mauao
#SBATCH --gres=gpu:1
#SBATCH --job-name=PG1-scale
#SBATCH --tasks-per-node=1
#SBATCH --output=%x-%j.out
#SBATCH --dependency=afterany:77892
...
for n_clients_per_round in "10000"; do
    for policy in "lb" "llb" "rr"; do
        ...
        CUSTOM_HYDRA_ARGS="num_nodes=1 run_uuid=$run_uuid task=google_speech task.n_clients_per_round=$n_clients_per_round task.num_rounds=100 local_epochs=1 placement_policy=$policy flwr_address=127.0.0.1:6481"
