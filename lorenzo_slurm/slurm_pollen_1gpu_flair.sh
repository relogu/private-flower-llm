...
for n_workers in $(seq 1 20); do
    policy="rr"
    # Get the timestamp and the unique run id
    timestamp=$(date +%Y-%m-%d_%H%M%S)
    run_uuid=conc-est-flair-1gpu-a40-$timestamp
    export RUN_UUID=$run_uuid
    export CONCURRENCY=$n_workers
    echo "Using policy $policy with $n_workers workers. Run UUID: $run_uuid"
    CUSTOM_HYDRA_ARGS="num_nodes=1 cap_num_workers_per_gpu=$CONCURRENCY run_uuid=$run_uuid task=flair task.num_rounds=50 placement_policy=$policy flwr_address=127.0.0.1:7482"
