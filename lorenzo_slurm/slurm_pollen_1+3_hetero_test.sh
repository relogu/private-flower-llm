...
run_uuid="PF-11-lb-1w-$timestamp"
...
for policy in "lb" "bu" "rr"; do
    ...
    CUSTOM_HYDRA_ARGS="num_nodes=2 cap_num_workers_per_gpu=1 run_uuid=$run_uuid task=flair task.n_clients_per_round=200 task.num_rounds=100 local_epochs=2 placement_policy=$policy flwr_address=$ip:7381"
