#!/bin/bash
#! Slurm requests
#SBATCH --job-name=R-npa-G13-5le
#SBATCH --nodelist=ngongotaha,mauao
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:3
#SBATCH --cpus-per-task=24
#SBATCH --output=%x-%j.out
#SBATCH --dependency=afterany:82053
...
CUSTOM_HYDRA_ARGS="num_nodes=2 run_uuid=$run_uuid task=google_speech task.n_clients_per_round=100 task.num_rounds=50 local_epochs=5 ray_address="auto" ray_redis_password=$redis_password ray_node_ip_address=$ip"
