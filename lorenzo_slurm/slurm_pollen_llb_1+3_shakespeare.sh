...
#SBATCH --gres=gpu:3
#SBATCH --job-name=PS13-llb
#SBATCH --cpus-per-task 24
#SBATCH --ntasks-per-node=1
#SBATCH --output=%x-%j.out
#!SBATCH --dependency=afterany:78085
...
CUSTOM_HYDRA_ARGS="num_nodes=2 run_uuid=$run_uuid task=shakespeare_memory task.n_clients_per_round=100 task.num_rounds=100 local_epochs=1 placement_policy=$policy flwr_address=$ip:6379"
