...
for policy in "lb"; do
    run_uuid="PF-13-$policy-mw-20000-$timestamp"
    echo "Using policy $policy"
    CUSTOM_HYDRA_ARGS="num_nodes=2 run_uuid=$run_uuid task=flair task.n_clients_per_round=20000 task.num_rounds=100 local_epochs=2 placement_policy=$policy flwr_address=$ip:7382"
