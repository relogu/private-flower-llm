...
#SBATCH --gres=gpu:3
#SBATCH --job-name=P-pa-O13-lb-5le
#SBATCH --cpus-per-task 24
#SBATCH --ntasks-per-node=1
#SBATCH --output=%x-%j.out
#!SBATCH --dependency=afterany:78109
...
CUSTOM_HYDRA_ARGS="num_nodes=2 run_uuid=$run_uuid task=openimage task.n_clients_per_round=100 task.num_rounds=50 local_epochs=5 placement_policy=$policy flwr_address=$ip:6380"
