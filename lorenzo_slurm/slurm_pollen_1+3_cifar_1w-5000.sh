...
#SBATCH --job-name=PC13-mw-5000
#SBATCH --dependency=afterany:2098
...
for policy in "lb"; do
    run_uuid="PC-13-$policy-mw-5000-$timestamp"
    echo "Using policy $policy"
    CUSTOM_HYDRA_ARGS="num_nodes=2 run_uuid=$run_uuid task=cifar10 task.n_clients_per_round=5000 task.num_rounds=250 local_epochs=1 placement_policy=$policy flwr_address=$ip:7381"
