...
#SBATCH -w mauao
#SBATCH --gres=gpu:1
#SBATCH --job-name=RG1-scale
#SBATCH --partition=normal
#SBATCH --tasks-per-node=1
#SBATCH --output=%x-%j.out
#SBATCH --dependency=afterany:77884
...
for n_clients_per_round in "1000" "10000"; do
  ...
  CUSTOM_HYDRA_ARGS="run_uuid=$run_uuid task=google_speech task.n_clients_per_round=$n_clients_per_round task.num_rounds=100 local_epochs=1 ray_address=auto ray_redis_password=$redis_password ray_node_ip_address=$ip"
