...
. "$PROJECT_PATH"/scripts/set_llm_config.sh "540M"
...
if [ -z "$RUN_UUID" ]; then
	export RUN_UUID="fed-540M-$DATETIME"
fi
...
POLLEN_CONFIG="$POLLEN_CONFIG llm_config.scheduler.t_max=88000ba llm_config.scheduler.t_warmup=1000ba llm_config.scheduler.alpha_f=0.1 llm_config.optimizer.lr=4.0e-4"
...
poetry run flower-superlink --insecure --driver-api-address '[::]:50756' --fleet-api-address '[::]:51756' 2>&1 | tee "$POLLEN_SAVE_PATH"/superlink.log &
...
# GRPC_VERBOSITY=debug
CUDA_LAUNCH_BLOCKING=1 poetry run flower-client-app flower_llm.node_manager.node_manager:client_app --insecure --superlink '[::]:51756' --persist-client 2>&1 | tee "$POLLEN_SAVE_PATH"/node_manager.log &
#! Keep the pid of the NodeManager
BACK_PID=$!

#! Launch ServerWithPollen as a ServerApp
# GRPC_VERBOSITY=debug
poetry run flower-server-app flower_llm.launch_pollen_server:server_app --insecure --superlink '[::]:50756' 2>&1 | tee "$POLLEN_SAVE_PATH"/server.log &
