...
# # GRPC_VERBOSITY=debug
# CUDA_LAUNCH_BLOCKING=1 poetry run flower-client-app flower_llm.client_app:app --insecure --superlink '[::]:51844' --persist-client 2>&1 | tee "$POLLEN_SAVE_PATH"/node_manager.log &
# #! Keep the pid of the NodeManager
# BACK_PID=$!

#! Launch ServerWithPollen as a ServerApp
# GRPC_VERBOSITY=debug
poetry run flower-server-app flower_llm.server_app:app --insecure --superlink '[::]:50844' 2>&1 | tee "$POLLEN_SAVE_PATH"/server.log &
