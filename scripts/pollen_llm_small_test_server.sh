...
if [ -z "$RUN_UUID" ]; then
	export RUN_UUID="fed-small-test-server-$DATETIME"
fi
...
poetry run flower-superlink --insecure --driver-api-address '[::]:50844' --fleet-api-address '[::]:51844' 2>&1 | tee "$POLLEN_SAVE_PATH"/superlink.log &
...
# # GRPC_VERBOSITY=debug
# CUDA_LAUNCH_BLOCKING=1 poetry run flower-client-app flower_llm.node_manager.node_manager:client_app --insecure --superlink '[::]:51844' --persist-client 2>&1 | tee "$POLLEN_SAVE_PATH"/node_manager.log &
# #! Keep the pid of the NodeManager
# BACK_PID=$!

#! Launch ServerWithPollen as a ServerApp
# GRPC_VERBOSITY=debug
poetry run flower-server-app flower_llm.launch_pollen_server:server_app --insecure --superlink '[::]:50844' 2>&1 | tee "$POLLEN_SAVE_PATH"/server.log &
