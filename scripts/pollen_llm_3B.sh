...
. "$PROJECT_PATH"/scripts/set_llm_config.sh "3B"
...
POLLEN_CONFIG="$POLLEN_CONFIG fl.server_learning_rate=0.7 fl.server_momentum=0.9"
...
poetry run flower-superlink --insecure --driver-api-address '[::]:50759' --fleet-api-address '[::]:51759' 2>&1 | tee "$POLLEN_SAVE_PATH"/superlink.log &
...
# GRPC_VERBOSITY=debug
CUDA_LAUNCH_BLOCKING=1 poetry run flower-client-app flower_llm.node_manager.node_manager:client_app --insecure --superlink '[::]:51759' --persist-client 2>&1 | tee "$POLLEN_SAVE_PATH"/node_manager.log &
#! Keep the pid of the NodeManager
BACK_PID=$!

#! Launch ServerWithPollen as a ServerApp
# GRPC_VERBOSITY=debug
poetry run flower-server-app flower_llm.launch_pollen_server:server_app --insecure --superlink '[::]:50759' 2>&1 | tee "$POLLEN_SAVE_PATH"/server.log &
