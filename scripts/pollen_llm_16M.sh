...
. "$PROJECT_PATH"/scripts/set_llm_config.sh "16M"
...
if [ -z "$RUN_UUID" ]; then
	export RUN_UUID="fed-16M-$DATETIME"
fi
...
N_LOCAL_STEPS=500
POLLEN_CONFIG="run_uuid=$RUN_UUID pollen.refresh_period=40 fl.n_rounds=176"
# NOTE: set dataset
...
poetry run flower-superlink --insecure --driver-api-address '[::]:50750' --fleet-api-address '[::]:51750' 2>&1 | tee "$POLLEN_SAVE_PATH"/superlink.log &
...
# GRPC_VERBOSITY=debug
CUDA_LAUNCH_BLOCKING=1 poetry run flower-client-app flower_llm.node_manager.node_manager:client_app --insecure --superlink '[::]:51750' --persist-client 2>&1 | tee "$POLLEN_SAVE_PATH"/node_manager.log &
#! Keep the pid of the NodeManager
BACK_PID=$!

#! Launch ServerWithPollen as a ServerApp
# GRPC_VERBOSITY=debug
poetry run flower-server-app flower_llm.launch_pollen_server:server_app --insecure --superlink '[::]:50750' 2>&1 | tee "$POLLEN_SAVE_PATH"/server.log &
