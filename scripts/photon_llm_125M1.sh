...
POLLEN_CONFIG="$POLLEN_CONFIG fl.n_rounds=10"           # Total number of rounds
...
POLLEN_CONFIG="$POLLEN_CONFIG llm_config.scheduler.t_max=5000ba"   # DiLoCo
POLLEN_CONFIG="$POLLEN_CONFIG llm_config.scheduler.t_warmup=100ba" # DiLoCo
POLLEN_CONFIG="$POLLEN_CONFIG llm_config.scheduler.alpha_f=0.1"    # DiLoCo
POLLEN_CONFIG="$POLLEN_CONFIG llm_config.optimizer.lr=6.0e-4"      # DiLoCo
...
POLLEN_CONFIG="$POLLEN_CONFIG llm_config.global_train_batch_size=32"         # DiLoCo single device batch size (one client simulates one device -> 512/8=64)
...
poetry run flower-superlink --insecure --driver-api-address '[::]:50753' --fleet-api-address '[::]:51754' 2>&1 | tee "$POLLEN_SAVE_PATH"/superlink.log &
SUPERLINK_PID=$!
sleep 5
...
# GRPC_VERBOSITY=debug
CUDA_LAUNCH_BLOCKING=1 poetry run flower-client-app flower_llm.client_app:app --insecure --superlink '[::]:51754' --persist-client 2>&1 | tee "$POLLEN_SAVE_PATH"/node_manager.log &
#! Keep the pid of the ClientApp
CLIENTAPP_PID=$!

#! Launch ServerWithPollen as a ServerApp
# GRPC_VERBOSITY=debug
poetry run flower-server-app flower_llm.server_app:app --insecure --superlink '[::]:50753' 2>&1 | tee "$POLLEN_SAVE_PATH"/server.log &
