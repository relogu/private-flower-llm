...
. "$PROJECT_PATH"/scripts/set_llm_config.sh "small"
...
# N_LOCAL_STEPS=500
N_LOCAL_STEPS=100
# POLLEN_CONFIG="run_uuid=$RUN_UUID pollen.refresh_period=50 pollen.cpu_only=true"
POLLEN_CONFIG="run_uuid=$RUN_UUID pollen.refresh_period=50"
# NOTE: set dataset
...
POLLEN_CONFIG="$POLLEN_CONFIG fl.strategy_name=NESTOROV fl.strategy_kwargs.server_learning_rate=0.1 fl.strategy_kwargs.server_momentum=0.9"
POLLEN_CONFIG="$POLLEN_CONFIG fl.n_total_clients=64 fl.n_clients_per_round=4 fl.n_rounds=176"
# POLLEN_CONFIG="$POLLEN_CONFIG fl.n_total_clients=64 fl.n_clients_per_round=1 fl.n_rounds=176"
...
# POLLEN_CONFIG="$POLLEN_CONFIG llm_config.eval_first=true llm_config.eval_interval=250ba"
POLLEN_CONFIG="$POLLEN_CONFIG llm_config.eval_first=true llm_config.eval_interval=50ba llm_config.eval_subset_num_batches=1"
...
POLLEN_CONFIG="$POLLEN_CONFIG use_wandb=true"
...
# GRPC_VERBOSITY=debug
CUDA_LAUNCH_BLOCKING=1 poetry run flower-client-app flower_llm.client_app:app --insecure --superlink '[::]:50749' --persist-client 2>&1 | tee "$POLLEN_SAVE_PATH"/node_manager.log &
...
# GRPC_VERBOSITY=debug
poetry run flower-server-app flower_llm.server_app:app --insecure --superlink '[::]:51749' 2>&1 | tee "$POLLEN_SAVE_PATH"/server.log &
