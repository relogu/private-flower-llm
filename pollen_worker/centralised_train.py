        ...
        if loader.name == "text":
            if cfg.model.name in ["hf_prefix_lm", "hf_t5"]:
                raise ValueError(
                    ...
                    " pre-train that model type."
                )
        elif loader.name == "text_denoising":
            if cfg.model.name == "hf_causal_lm":
                raise ValueError(
                    f'Model type "{cfg.model.name}" is not supported when using the'
                    ' "text_denoising" dataloader. Please use the "text" dataloader to'
                    " pre-train that model type."
                )
            if (
...
def build_composer_model(
    ...
    """Build a Composer model from a model config and tokenizer."""
...
def main(_cfg: DictConfig) -> Trainer:
    ...
    warnings.filterwarnings(
        ...
        message="torch.distr*_base is a private function and will be deprecated.*",
    )
