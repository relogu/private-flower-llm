...
class ServerState(object):
    ...
    def __init__(
        ...
        if not isinstance(elapsed_time_in_seconds, float) and not isinstance(
        ...
        self.elapsed_time_in_seconds: float = (
            ...
            if isinstance(elapsed_time_in_seconds, float)
            else float(elapsed_time_in_seconds)
        )
...
class ServerStateWithGlobalModel(ServerState):
    """Class for the server state with a global model."""
